...
def convolve2d(img, kernel):
    ...
    for i in range(1,R):
        for j in range(1,C):
            imgcropped= utils.crop(img, i-1, i+2, j-1, j+2)
            img_conv1= utils.elementwise_mul(imgcropped, kernel)
         
            convsum=[]
            for a in img_conv1:
                convsum.append(sum(a))
               
            img_conv[i][j]= sum(convsum)
            del convsum
    ...
    for i in range(R):
        for j in range(C):
            if maximum==minimum:
                ...
                img_normal[i][j]= (img[i][j]-minimum)/(maximum-minimum)
...
def edge_magnitude(edge_x, edge_y):
    ...
    """
    # TODO: implement this function.
    xsquare= utils.elementwise_mul(edge_x,edge_x)
    ysquare= utils.elementwise_mul(edge_y,edge_y)
    q= utils.elementwise_add(xsquare,ysquare)
    edge_mag= np.sqrt(q)
